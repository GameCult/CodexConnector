...
The current source establishes the v2 typed contract before adding the daemon.
This is deliberate: the existing Yggdrasil connector already proves the
process seam, but its single-caller, fixed-model, stateless, no-tools wire law
cannot become the shared contract by inertia.
